...
#include "hho-magnetostatics-alt.hpp"
...
int main(int argc, const char* argv[])
{
  ...
  double err_curl = compute_curl_error(pb, b, uI, use_threads);   //----->check id change needed
  double err_grad = compute_grad_error(pb, grad_p, pI, use_threads);//------>this should be fine
                                               ...
                                               )
{
  ...
  for (size_t iF = 0; iF < T.n_faces(); iF++) {
    ...
    auto xspace_basis1_F_quad = evaluate_quad<Function>::compute(xSpace().faceBasis1(F.global_index()), quad_2kpo_F);
    auto xspace_basis2_F_quad = evaluate_quad<Function>::compute(xSpace().faceBasis2(F.global_index()), quad_2kpo_F);

    //for xspace contribution face contribution is split in two: 4 blocks to fill instead of 1
    AT.block(zSpace().localOffset(T, F, 0), zSpace().localOffset(T, F, 0), xSpace().face_basis1_dim(), xSpace().face_basis1_dim())
      += 1./hF * compute_gram_matrix(xspace_basis1_F_quad, quad_2kpo_F);
    AT.block(zSpace().localOffset(T, F, 0), zSpace().localOffset(T, F, 0)+xSpace().face_basis1_dim(), xSpace().face_basis1_dim(), xSpace().face_basis2_dim())
      += 1./hF * compute_gram_matrix(xspace_basis1_F_quad, xspace_basis2_F_quad, quad_2kpo_F);
    AT.block(zSpace().localOffset(T, F, 0)+xSpace().face_basis1_dim(), zSpace().localOffset(T, F, 0), xSpace().face_basis2_dim(), xSpace().face_basis1_dim())
      += 1./hF * compute_gram_matrix(xspace_basis2_F_quad, xspace_basis2_F_quad, quad_2kpo_F);
    AT.block(zSpace().localOffset(T, F, 0)+xSpace().face_basis1_dim(), zSpace().localOffset(T, F, 0)+xSpace().face_basis1_dim(),
                      xSpace().face_basis2_dim(), xSpace().face_basis2_dim())
      += 1./hF * compute_gram_matrix(xspace_basis2_F_quad, quad_2kpo_F);

    //for xspace constribution cell-face block is split into two block
    Eigen::MatrixXd hFmo_xspace_MFT = Eigen::MatrixXd::Zero(xSpace().numLocalDofsFace(), xSpace().numLocalDofsCell());
    hFmo_xspace_MFT.block (0,0,xSpace().numLocalDofsFace(), xSpace().face_basis1_dim())
    += 1./hF * compute_gram_matrix(xspace_basis1_F_quad, xspace_basis_T_quad, quad_2kpo_F);
    hFmo_xspace_MFT.block (0,xSpace().face_basis1_dim(), xSpace().numLocalDofsFace(), xSpace().face_basis2_dim())
    += 1./hF * compute_gram_matrix(xspace_basis2_F_quad, xspace_basis_T_quad, quad_2kpo_F);
