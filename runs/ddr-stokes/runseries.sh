...
echo -e "Deg MeshSize NbCells NbFaces NbEdges DimXCurl DimXGrad E_HcurlVel Rate E_HgradPre Rate E_L2Vel E_L2CurlVel E_L2Pre E_L2GradPre" > $outsubdir/$errorsfile
...
do
    ...
    E_HgradPre=$(awk '/E_HgradPre:/ {print $NF}' $outsubdir/results-$i.txt)
    E_L2Vel=$(awk '/E_L2Vel:/ {print $NF}' $outsubdir/results-$i.txt)
    E_L2CurlVel=$(awk '/E_L2CurlVel:/ {print $NF}' $outsubdir/results-$i.txt)
    ...
    if(($i \> 1)); then
      ...
      OldE_HgradPre=$(awk '/E_HgradPre:/ {print $NF}' $outsubdir/results-$imo.txt)
      HgradPreRate=$(perl -E "say sprintf(\"%.2f\", log($OldE_HgradPre/$E_HgradPre)/log($OldMeshSize/$MeshSize))")
      echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimXCurl $DimXGrad $E_HcurlVel $HcurlVelRate $E_HgradPre $HgradPreRate $E_L2Vel $E_L2CurlVel $E_L2Pre $E_L2GradPre" >> $outsubdir/$errorsfile
    else
  	  echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimXCurl $DimXGrad $E_HcurlVel -- $E_HgradPre -- $E_L2Vel $E_L2CurlVel $E_L2Pre $E_L2GradPre" >> $outsubdir/$errorsfile
    fi
done;
# For standard output: we do not output all the separate errors for u, p etc.
cat $outsubdir/$errorsfile | cut -d ' ' -f 1-11 | column -t
