...
do
	...
	# Execute code
	$executable -t $meshtype -m $meshfile -k $k -s -bc $bc
  # Move outputs
...
cd $outdir
echo -e "MeshSize EnergyError L2Error MeshReg NoDofs" > $errorsfile
for i in `seq 1 $nbmesh`;
do
  ...
  NoDofs=$(awk '/NoDofs:/ {print $NF}' results-$i.txt)
	echo -e "$MeshSize $EnergyError $L2Error $MeshReg $NoDofs" >> $errorsfile
done;
  ...
  \\\vspace{0.5cm}\\\\\\
    ...
    \\\begin{tikzpicture}
      \\\begin{loglogaxis}[ legend columns=-1, legend to name=legend ]
        ...
        \\\addlegendentry{Energy Error};
        \\\addplot table[x=MeshSize,y=L2Error] {data_rates.dat};
        \\\addlegendentry{\$L2\$ Error};
    ...
    \\\begin{tikzpicture}
      \\\begin{loglogaxis}
        \\\addplot table[x=NoDofs,y=EnergyError] {data_rates.dat};
        \\\addplot table[x=NoDofs,y=L2Error] {data_rates.dat};
      \\\end{loglogaxis}
   \\\end{tikzpicture}
  \\\caption{Errors vs. DOFS}
